...
def test_charges_and_payments_compute_balance(client):
    ...
    client.post(f"/api/v1/bookkeeping/companies/{cid}/entries", json={
        "entry_date": "2026-08-01", "type": "charge", "amount": "150.00", "invoice_number": "flour order",
    })
    client.post(f"/api/v1/bookkeeping/companies/{cid}/entries", json={
...
def test_receivable_company_balance_same_formula(client):
    ...
    client.post(f"/api/v1/bookkeeping/companies/{cid}/entries", json={
        "entry_date": "2026-08-01", "type": "charge", "amount": "500.00", "invoice_number": "custom cake order",
    })
...
def _company_with_entry(client, **entry):
    ...
    body = {"entry_date": "2026-08-01", "type": "charge", "amount": "150.00", "invoice_number": "flour"}
    body.update(entry)
...
def test_editing_one_field_does_not_blank_the_others(client):
    ...
    cid, eid = _company_with_entry(client)
    r = client.put(f"/api/v1/bookkeeping/companies/{cid}/entries/{eid}", json={"invoice_number": "flour + rye"})

    entry = r.json()["entries"][0]
    assert entry["invoice_number"] == "flour + rye"
...
def test_a_note_can_be_cleared_explicitly(client):
    cid, eid = _company_with_entry(client)
    r = client.put(f"/api/v1/bookkeeping/companies/{cid}/entries/{eid}", json={"invoice_number": None})
    assert r.json()["entries"][0]["invoice_number"] is None
