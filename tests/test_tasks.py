...
def test_manager_creates_task_and_overdue_flag(client, make_user):
    ...
    r = client.post("/api/v1/tasks", json={
        "title": "Restock flour", "description": "Restock flour", "assigned_to": uid, "due_date": past})
...
def test_cashier_cannot_create_task(client, make_user):
    ...
    r = cashier.post("/api/v1/tasks", json={"title": "x", "assigned_to": 1})
...
def test_employee_sees_only_own_tasks(client, make_user):
    ...
    client.post("/api/v1/tasks", json={"title": "A", "assigned_to": tara_id})
    client.post("/api/v1/tasks", json={"title": "B", "assigned_to": uma_id})
...
def test_assignee_can_toggle_done(client, make_user):
    ...
    tid = client.post("/api/v1/tasks", json={
        "title": "Sweep", "assigned_to": vic_id}).json()["id"]
...
def test_other_cashier_cannot_complete_task(client, make_user):
    ...
    tid = client.post("/api/v1/tasks", json={
        "title": "Mop", "assigned_to": will_id}).json()["id"]
...
def test_done_task_not_overdue(client, make_user):
    ...
    past = (datetime.now(timezone.utc) - timedelta(hours=1)).isoformat()
    tid = client.post("/api/v1/tasks", json={
        "title": "Old task", "assigned_to": yid, "due_date": past}).json()["id"]
